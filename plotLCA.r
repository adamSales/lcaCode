library(ggplot2)
### tweaking plot function from polca

plotLCA <- function (x, ord=NULL,classNames=NULL,...)
{
    K.j <- sapply(x$probs, ncol)
    R <- length(x$P)
    if(is.null(ord)) ord <- 1:R
    if(is.null(classNames)) classNames <- paste("Class",1:R)
    if (max(K.j) == 2) {
        poLCA.makeplot.dich(x$probs, x$P, x$y, NULL)
    }
    else {
        layout(cbind(c(seq(R+1),0)), heights = c(rep(5,R), 1))
        for (r in ord) {
            makeplot(x$probs, r, x$y, K.j,
                     paste0(classNames[r],":\n population share = ", round(x$P[r], 3)),...)
...
makeplot<-
function (probs, r, y, K.j, ti,xlab = "Manifest variables", zlab = "Pr(outcome)",varnames=colnames(y),categories=NULL)
{
    pi.class <- matrix(NA, nrow = length(probs), ncol = max(K.j))
    for (j in 1:length(probs)) {
        pi.class[j, 1:K.j[j]] <- probs[[j]][r, ]
    }
    if(is.null(categories)) categories <- as.character(c(1:max(K.j)))
    dimnames(pi.class) <- list(as.character(c(1:ncol(y))), categories)
    ds.plot <- data.frame(Manifest.variables = as.vector(row(pi.class)),
        Outcomes = as.vector(col(pi.class)), value = as.vector(pi.class))
    names(ds.plot)[2] <- ''
    vis <- scatterplot3d(ds.plot, type = "h", lwd = 5, pch = " ",
                         x.ticklabs = '',#varnames,
                         y.ticklabs = colnames(pi.class),
                         z.ticklabs = " ", xlab=xlab,zlab=zlab,
                                        #main = ti,
                         cex.main = 1.5, color = 2,
                         lab = c(ncol(y) - 1, max(K.j) - 1), zlim = c(0, 1), box = FALSE, angle = 75,
                         mar = c(3, 3, 2, 3.5))
    usr <- par('usr')
    text((usr[2]-usr[1])/2,usr[4],ti,xpd=TRUE,cex=1.5)
    text(seq(length(K.j)), par("usr")[3] - 0.2, labels = varnames, srt = 45, pos = 1, xpd = TRUE)
}
